...
def load_model(state, snapshot_path):
    """Don't save optimizer for less storage.
    """
    snapshot = torch.load(snapshot_path)
    state.generator.load_state_dict(snapshot['generator_state_dict'])
    state.discriminator.load_state_dict(snapshot['discriminator_state_dict'])
    if hasattr(state, 'rnn'):
        state.rnn.load_state_dict(snapshot['rnn_state_dict'])
        # state.rnn_optimizer.load_state_dict(
        #     snapshot['rnn_optimizer_state_dict'])
    state.layer_norm.load_state_dict(snapshot['layer_norm_state_dict'])
...
def save_model(state, path, epoch, iteration):
    ...
    if hasattr(state, 'rnn'):
        snapshot['rnn_state_dict'] = state.rnn.state_dict()
        # snapshot['rnn_optimizer_state_dict'] = state.rnn_optimizer.state_dict()
    if hasattr(state, 'image_encoder'):
        snapshot['image_encoder_state_dict'] = state.image_encoder.state_dict()
...
def _plot_gradients(state, visualizer, rnn, gen, disc, gru, ce, ie, iteration):
    visualizer.plot('ConditionEncoder Gradient Norm', 'train', iteration, ce)
    visualizer.plot('ImageEncoder Gradient Norm', 'train', iteration, ie)
    visualizer.plot('RNN Gradient Norm', 'train', iteration, rnn)
    if gru:
        visualizer.plot('GRU Gradient Norm', 'train', iteration, gru)

    visualizer.plot('Generator Gradient Norm', 'train', iteration, gen)
    visualizer.plot('Discriminator Gradient Norm', 'train', iteration, disc)


def _plot_losses(state, visualizer, g_loss, d_loss, aux_loss, iteration):
    visualizer.plot('Discriminator output', 'train', iteration, d_loss)
    visualizer.plot('Generator output', 'train', iteration, g_loss)
    if state.cfg.aux_reg > 0:
        visualizer.plot('Aux Loss', 'train', iteration, aux_loss)
