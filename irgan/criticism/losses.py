...
class HingeAdversarial():
    ...
    @staticmethod
    def discriminator(real, fake, wrong=None, wrong_weight=0., mask=None):
        ...
        """
        if mask is not None:
            l_real = (F.relu(1. - real)*mask).sum()/mask.sum()
            l_fake = (F.relu(1. + fake)*mask).sum()/mask.sum()
        else:
        ...
        if wrong is None:
            ...
            l_wrong = F.relu(1. + wrong*mask).sum()/mask.sum()
