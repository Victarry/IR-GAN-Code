...
class VisdomPlotter():
    ...
    def draw(self, var_name, images, nrow=8):
        ...
        if var_name not in self.plots:
            ...
            self.viz.images(images, env=self.env, win=self.plots[var_name], nrow=nrow, opts=dict(caption=var_name))
...
class TensorboardPlotter(VisdomPlotter):
    def __init__(self, env_name, server):
        super().__init__(env_name, server)
        self.writer = SummaryWriter(comment=env_name)
    ...
    def plot(self, var_name, split_name, x, y, xlabel='iteration'):
        ...
        Args:
            ...
            - xlabel: label of the x_axis (str)
        """
        super().plot(var_name, split_name, x, y, xlabel)
        self.writer.add_scalar(var_name, y, global_step=x)
