...
# Global Keys
with open('irgan/config.yml', 'r') as f:
    keys = yaml.load(f)
...
def parse_config(args=None):
    ...
    parser.add_argument('-use_history',
                        action='store_true',
                        help='a flag which decide whether to use history'
                        'instruction features for the condition encoder')

    parser.add_argument('-use_noise',
                        action='store_true',
                        help='a flag which decide whether to use noise'
